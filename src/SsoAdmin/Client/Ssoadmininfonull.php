...
use xltxlm\ssoclient\SsoAdmin\ssoadminModel;
...
class Ssoadmininfonull
{
    ...
    */
    public function setThriftConfig(ThriftConfig $ThriftConfig )
    {
        ...
        if ($ThriftConfig->getType() == ThriftConfig::HTTP) {
            $socket = new THttpClient($ThriftConfig->getHost(), $ThriftConfig->getPort(), '?c=ThriftServer/ThriftServerSsoadmininfonull',$_SERVER['HTTPS'] ? 'https' : 'http');
    ...
    */
    public function __invoke()  {
                return call_user_func_array( [$this->client,'ssoadmininfonull'], [$this->getSsoadminModel()]);
    }
