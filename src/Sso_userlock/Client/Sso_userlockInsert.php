...
class Sso_userlockInsert
{
    ...
    */
    public function setThriftConfig(ThriftConfig $ThriftConfig )
    {
        ...
        if ($ThriftConfig->getType() == ThriftConfig::HTTP) {
            $socket = new THttpClient($ThriftConfig->getHost(), $ThriftConfig->getPort(), '?c=ThriftServer/ThriftServerSso_userlockInsert',$_SERVER['HTTPS'] ? 'https' : 'http');
    ...
    */
    public function __invoke()  {
                return call_user_func_array( [$this->client,'Sso_userlockInsert'], [$this->getSsoUserlockModel()]);
    }
